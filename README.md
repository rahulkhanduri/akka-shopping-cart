...
```
sbt "runMain cnapp.poc.infrastructure.Main cassandra"
```

2. Start a node that runs the write model:

```
sbt -Dakka.cluster.roles.0=write-model "runMain cnapp.poc.infrastructure.Main 2551"
```

3. Start a node that runs the read model:

```
sbt -Dakka.cluster.roles.0=read-model "runMain cnapp.poc.infrastructure.Main 2552"
```

4. More write or read nodes can be started started by defining roles and port:

```
sbt -Dakka.cluster.roles.0=write-model "runMain cnapp.poc.infrastructure.Main 2553"
sbt -Dakka.cluster.roles.0=read-model "runMain cnapp.poc.infrastructure.Main 2554"
```
