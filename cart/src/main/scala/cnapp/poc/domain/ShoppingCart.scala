...
object ShoppingCart {
  ...
  final case class CheckedOut(cartId: String, eventTime: Instant,summary: Summary) extends Event
    ...
    command match {
      ...
      case Checkout(replyTo) =>
        if (state.isEmpty)
          ...
          Effect
            .persist(CheckedOut(cartId, Instant.now(),state.toSummary))
            .thenReply(replyTo)(updatedCart => Accepted(updatedCart.toSummary))
        }
  ...
  private def handleEvent(state: State, event: Event) = {
    event match {
      ...
      case CheckedOut(_, eventTime,_)                  => state.checkout(eventTime)
    }
  }
}
