...
import cnapp.poc.api.ShoppingCartRoutes
import cnapp.poc.domain.{EventProcessorSettings, ShoppingCart}
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    args.headOption match {
      ...
      case Some(portString) if portString.matches("""\d+""") =>
        ...
        val prometheusPort =("90" + portString.takeRight(2)).toInt
  ...
  def startNode(port: Int, httpPort: Int,prometheusPort:Int): Unit = {
     val system =
      ActorSystem[Nothing](Guardian(), "Shopping", config(port, httpPort,prometheusPort))
...
object Guardian {
  ...
  def createProjectionFor(
    ...
    CassandraProjection.atLeastOnce(
      ...
      sourceProvider,
      handler = () => new ShoppingCartProjectionHandler(tag, system,MongodbRepository.getRepository(system)("shopping")))
  }
