...
import cnapp.poc.domain.ShoppingCart
import cnapp.poc.helper.InventoryHelper
        ...
        concat(
          post {
            entity(as[AddItem]) { data =>
              ...
              val validation:Future[Boolean] = InventoryHelper.isAvailable(data.itemId,data.quantity)
              onSuccess(validation) {
                case true => {
                  val reply: Future[ShoppingCart.Confirmation] = entityRef.ask(ShoppingCart.AddItem(data.itemId, data.quantity, _))
                  onSuccess(reply) {
                ...
                case false => complete(StatusCodes.BadRequest -> "validation error")
              }
            }
          },
          put {
